...
class TestGetCourseCurriculum:
    ...
    def test_requests_asset_type_and_download_urls(self):
        api = _make_api()
        page = {"results": [], "next": None}
        with patch.object(api.session, "get", return_value=_mock_response(page)) as mock_get:
            api.get_course_curriculum(12345)
        called_url = mock_get.call_args[0][0]
        assert "asset_type" in called_url
        assert "download_urls" in called_url
        assert "filename" in called_url
        assert "body" in called_url
        assert "external_url" in called_url
