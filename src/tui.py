...
class TUI:
    ...
    def safe_addstr(self, y: int, x: int, text: str, color: int = 0, 
                    attr: int = 0, max_width: Optional[int] = None):
        try:
    ...
    def draw_progress_bar(self, y: int, x: int, width: int, percent: float, 
                         prefix: str = "", suffix: str = "", color: int = COLOR_ACCENT):
    ...
    def render_dashboard(self, state: Dict[str, Any], course_index: int, 
                        total_courses: int, log: List[str]):
        self.stdscr.clear()
        ...
        self.safe_addstr(0, 0, f"+-- Downloading Course [{course_index}/{total_courses}] ".ljust(width-1, "-") + "+", COLOR_ACCENT)
        self.safe_addstr(1, 2, f"Course : {state.get('course_title', '')[:width-20]}", COLOR_DEFAULT, curses.A_BOLD)
        overall_pct = (state['done_vids'] / state['total_vids'] * 100) if state['total_vids'] > 0 else 0
        self.draw_progress_bar(2, 2, width - 4, overall_pct, "Total  :", 
                              f"{overall_pct:3.0f}% [{state['done_vids']:03d}/{state['total_vids']:03d}]", COLOR_SUCCESS)
        
        self.safe_addstr(5, 2, f"File   : {state['current_file'][:width-20]}", COLOR_DEFAULT, curses.A_BOLD)
        ...
        for idx, line in enumerate(log[-(height - log_start_y - 1):]):
            color = COLOR_ERROR if "error" in line.lower() else (COLOR_SUCCESS if "done" in line.lower() else COLOR_DIM)
    ...
    def show_error(self, message: str):
        self.stdscr.clear()
        height, width = self.stdscr.getmaxyx()
        self.safe_addstr(height // 2, max(0, (width - 40) // 2), f"ERROR: {message}", COLOR_ERROR, curses.A_BOLD)
    ...
    def edit_settings(self, config: Config) -> None:
        ...
        while True:
            ...
            self.safe_addstr(height-2, 0, "| [Enter] Edit | [Q] Back ".ljust(width-1, " ") + "|", COLOR_DEFAULT)
            self.stdscr.refresh()
            ch = self.stdscr.getch()
            if ch == curses.KEY_UP and selected_idx > 0:
                ...
                self.safe_addstr(height-3, 0, prompt.ljust(width-1, " ")[:width-2] + "|", COLOR_WARN)
                try:
    ...
    def select_courses(self, courses: List[Dict]) -> List[Dict]:
        ...
        while True:
            ...
            self.safe_addstr(1, 0, "| [Space] Toggle | [Enter] Confirm | [Q] Cancel ".ljust(width-1, " ") + "|", COLOR_DEFAULT)
            ...
            for i in range(visible_items):
                ...
                self.safe_addstr(i + 3, 0, f"| {prefix} {box} {course['id']:<10} {title}", color, 0, width - 2)
            self.stdscr.refresh()
            k = self.stdscr.getch()
    ...
    def show_help(self):
        ...
        self.safe_addstr(height-2, 0, "| > Press any key to close".ljust(width-1, " ") + "|", COLOR_DIM)
        self.stdscr.refresh()
        self.stdscr.getch()
