...
The core download loop lives in :class:`DownloadPipeline`.  Both the
interactive TUI and the headless CLI delegate to this class, passing a
:class:`ProgressReporter` implementation that handles presentation.
...
class ProgressReporter(Protocol):
    ...
    Implemented by both the TUI dashboard and the headless console printer.
    """

    def on_log(self, message: str) -> None:
        """Called when a human-readable log line should be shown."""
        ...
...
class DownloadPipeline:
    """Orchestrates course downloads with pluggable progress reporting.

    Args:
        config: Application configuration.
        api: Authenticated Udemy API client.
        downloader: Video / material downloader instance.
        state: Application state manager for resume support.
        reporter: Callback interface for progress updates.
    """
    ...
    def download_courses(self, courses: List[Course]) -> bool:
        ...
        Returns ``True`` if all courses completed, ``False`` if
        interrupted.
        """
    ...
    def _download_course(self, course: Course, index: int, total: int) -> None:
        """Download all lectures in a single course."""
    ...
    def _build_download_queue(
        ...
        """Fetch the curriculum and build an ordered list of lectures."""
    ...
    def _download_lecture(
        ...
        """Download a single lecture (video + extras)."""
